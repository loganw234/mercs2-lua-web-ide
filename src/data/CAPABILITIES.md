...
| Namespace | What it's for | Key calls |
|---|---|---|
| `Ess.Camera` | Camera effects (top-level `Camera` + `Graphics.Camera` + `Graphics.Effect`, kept clear) | `.shake/.stopShake`, `.fov/.restoreFov`, `.fade(amt)` (+ `Easy.Camera.shake/fadeOut/fadeIn`), `.lookAtAnchor`, `.followHardpoint`, `.staleAxisDecay`; **cinematic:** `.beginCinematic/.placeCamera/.lookAtObject/.lookAtPoint/.hold/.endCinematic/.panicRevert` + `Easy.Camera.watch(uGuid, {chase=, angle=})` (locked-off tracking shot by default, or a `Blend 0` fixed-angle follow) + `Easy.Camera.orbit(uGuid, {radius=, speed=, smooth=})`. **chase/orbit damp the follow through `Ess.Vec.lerp` by default** (jitter-free on a fast subject; `smooth=false` / `smoothFactor` to tune). Steals control until `stop()` |
