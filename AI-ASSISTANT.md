...
Design notes for the IDE's provider-agnostic AI assistant panel, its agent mode,
and the grounding check that keeps them honest.

This began as a separate repo (`mercs2-lua-web-ide-ai`) so the assistant could be
built and tested without destabilising the IDE people actually use. That testing
is through and it now lives here; the fork remains as a fallback rather than an
active branch, and this file is the record of why the assistant works the way it
does. Everything below describes the merged IDE.
