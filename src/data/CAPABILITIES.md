...
| `Ess.Sys` | Level, build and environment facts | `.level()`, `.version()`, `.platform()`, `.language()`, `.uptime()`, `.isLoading()`, `.settings()`, `.build()` — the "what am I running on" reads, in one place rather than eight scattered getters |
...
| Namespace | What it's for | Key calls |
|---|---|---|
| `Ess.Atmosphere` | Sky, light and time-of-day — and the region system that keeps overwriting them | `.get/.set/.setColor` (values are TRANSACTIONAL: `GetValue` returns nil outside a Begin/End pair, which every call here handles for you), `.setTime/.setTimeSpeed`, `.blending/.setting/.region`, `.KEYS/.COLOR_KEYS/.REGIONS`. ⚠ Keys are **not validated** — a typo reads 0 forever, and the engine misspells its own (`fBloomContastMultiplier`), so use `.KEYS` rather than typing them. Crossing into one of the **40** atmosphere regions starts a ~1s blend toward that region's authored preset, which is why a change "does not stick": **freezing the cycle (`setTimeSpeed(0)`) is reliably map-wide, forcing a specific time is not** |

The beginner verbs for the same thing live in `Ess.Easy.World` (`.tint/.brightness/.hellscape/.resetAtmosphere`,
`.lockTimeOfDay/.night/.day/.freezeTime`) — see the one-liners table above. This is the tier underneath them.

## Audio & HUD

| Namespace | What it's for | Key calls |
|---|---|---|
| `Ess.Sound` | Sound/ambience cueing, plus the mixer | **cue:** `.cue/.stop/.ambience/.stopAmbience/.volume`; `Ess.Easy.Sound.play(cue)`. **validate:** `.duration(cue)` (seconds, `-1` = looping, nil = not resolvable), `.isCue/.isLooping` — a mistyped cue is otherwise completely silent. **mixer:** `.categoryVolume/.setCategoryVolume`, `.categoryPitch/.setCategoryPitch`, `.duck/.unduck/.clearDucking`, `.info()`, `.CATEGORIES` (sfx/music/ambience/vo). ⚠ get returns the EFFECTIVE volume (own × parent) while set takes the OWN level, and `ambience` is a child of `sfx` — restoring a category to a remembered *reading* leaves it quieter than it started |
| `Ess.Hud` | Native HUD popups and readouts | `.hint/.hideHint` (tutorial-style), `.banner(msg)` (centered text), `.objective(text)` (the persistent objective-tray line; nil clears), `.radio(text, hold)` (a self-clearing radio-chatter subtitle), `.title(text, dur, opts)` (the stylised animated overlay — the best-looking text the game has), `.location(text, dur)` (the region-name card), `.message(text, opts)` → handle + `.updateMessage/.removeMessage/.clearMessages` (negative duration = permanent), `.tutorial(text)` (sticky; nil clears), `.image(tex, slot)`, `.cash/.fuel/.resources/.suppressResources` (display only — these move the readout, not the money), `.RADAR_ICONS` |
| `Ess.Hud.Faction` | The faction meters, pursuit gauge, and the game's **only on-screen countdown** | `.timer(faction, secs, fn)` (real HUD chrome + a callback on expiry), `.pursuit(faction, secs, fn)` (the red gauge — it FILLS, it does not drain), `.add/.set/.inZone/.hide`, `.levels(thresholds, names, …)` ⚠ **destructive and global — it replaces the game's own faction mood vocabulary for every faction** — and `.restoreLevels()` to undo it. Values are 0..100; meters self-expire ~5s after their last update and cannot be pinned open |
| `Ess.Minimap` | The minimap **widget**, which no `Hud.*` function reaches | `.range(n)` (one-shot — the game recomputes it from player speed), `.lockRange(n)/.unlockRange()` (owns the update handler so it sticks), `.autoZoom(opts)` (retune the game's own speed curve), `.rotation/.show/.border`, `.widget()`. Re-apply after a level load — the widget is rebuilt |
| `Ess.Gps` | The player's map beacon | `.set(x, z)/.clear()/.get()/.distance()`, `.onSet(fn)/.onClear(fn)`. Not a routing system: the whole feature is one radar objective. There is **no `Player.SetGPS`** — a script-placed beacon is cosmetic, the engine does not treat it as the destination |
| `Ess.Pda` | The PDA — the mission log, database, statistics and map | **record:** `.log(msg, opts)` (the running journal — the one way to leave a permanent, player-reviewable trace), `.dossier`, `.stat/.statCategory`. **missions:** `.mission(name, opts)` (a blip naming a registered mission becomes a real mission blip and inherits its icon/label), `.missionExists`, `.removeMission` (cascades to its blips), `.selectMission/.trackable/.onMissionTrack/.allowMissionChange`. **map:** `.blip/.removeBlip`, `.region/.removeRegion`, `.fakePlayerLocation`, `.suppress`, `.ICONS`, `.FACTIONS` |
| `Ess.Shop` | The game's own full-screen **purchase UI**, filled with your items | `.open(items, {onBuy=, onClose=})` (one call for the native six-call sequence), `.close()`, `.isOpen()`, `.ICONS`. `nOwned`/`nCap` are the player's STOCKPILE, not shelf stock. `.close()` doubles as an **escape hatch** — the modal can otherwise strand the player with no input |
...
| `Ess.UI.Theme` | The whole look of the kit as ~36 plain values | `.preset(name)` (classic/cyan/slate/amber/neon/mono/dusk), `.get(k)` (read the EFFECTIVE value — reading the table directly gives nil for anything unset), `.apply()` (push + redraw what is already on screen), `.reset()`, `.DEFAULTS`, `.PRESETS`. Every widget is drawn at runtime from these, so restyling is assigning numbers — no Flash tools, no re-export. Set values before the first widget and no `.apply()` is needed |
